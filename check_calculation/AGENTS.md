# Instructions For `referee_agent`

## Summary
The audit is manual in judgment and report-writing, with tooling used only to generate evidence. The workflow is:
...
The report is not an auto-checker output. It is a hand-curated ledger backed by generated evidence stored in `check_calculation/`.
...
- "Ladder diagrams dominate" must be treated cautiously:
  - support it mechanically through explicit low-order evidence
  - then discuss it as a structural extrapolation used by the paper
  - do not overstate it as a brute-force all-orders proof
